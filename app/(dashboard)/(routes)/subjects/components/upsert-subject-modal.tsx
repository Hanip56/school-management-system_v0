...
const UpsertSubjectsModal = ({ open, handleClose, initialData }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof SubjectSchema>) => {
    try {
      ...
      if (initialData) {
        // update
        await axios.put(`/api/subject/${initialData.id}`, body);
      } else {
        // create
        await axios.post(`/api/subject`, body);
      }
  ...
  return (
    <Modal
      title={initialData ? "Update Subject" : "Create Subject"}
